...
interface UserContextType {
  ...
  valueHandel: (data: string) => void;
}

// 2. Create context with initial undefined and proper typing
const UserData = createContext<UserContextType | string>(undefined);
...
const UserDataProvider: React.FC<UserDataProviderProps> = ({ children }) => {
  ...
  const valueHandel = (data: string) => {
    setUserValue(data);
  };

  return (
    <UserData.Provider value={{ userValue, valueHandel }}>
      {children}
    </UserData.Provider>
  );
};
