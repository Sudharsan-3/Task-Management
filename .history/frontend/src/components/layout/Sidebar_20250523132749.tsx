      ...
      >
        ...
        <div className="p-4 border-t border-gray-700">
          <div className={`flex items-center ${isCollapsed && !isMobile ? 'justify-center' : ''}`}>
            <div onClick={clickHandel} className="w-10 h-10 rounded-full bg-gray-600 flex items-center justify-center ho ">
              <FaUserCircle className="text-2xl" />
            </div>
            {!isCollapsed && !isMobile && user && (
              <div className="ml-3 overflow-hidden">
                <p onClick={clickHandel} className="font-medium truncate ho ">{user.name}</p>
                <p onClick={clickHandel} className="text-xs text-gray-400 truncate ho ">{user.email}</p>
              </div>
            )}
          </div>
        </div>
