...
interface Task {
  id: number;
  status: 'draft' | 'pending' | 'completed' | 'notcompleted';
  [key: string]: any;
}
...
const Admin: React.FC = () => {
  ...
  const notcompleted = allTasks.filter((t) => t.status === 'notcompleted').length;
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="bg-white shadow-xl rounded-2xl p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              ...
              <MiniPieChart label="Notcompleted" count={notcompleted} color={COLORS.completed} />
            </div>
          </div>
        </div>
      )}
