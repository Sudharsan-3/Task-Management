...
const Admin: React.FC = () => {
 ...
 console.log(user,"from Admin")
  ...
  return (
    <div classnameName="min-h-screen flex flex-col items-center justify-start p-8 bg-gray-100">
      <h1 classnameName="text-4xl font-bold mb-8 text-gray-800">Welcome to Admin Page</h1>

      {notification && (
        <div classnameName="mb-4 px-4 py-2 bg-red-100 text-red-700 rounded-md">
          {notification}
        </div>
      )}

      <div classnameName="flex flex-wrap gap-6 justify-center">
        <button
          onClick={clickHandler}
          value="users"
          classnameName="px-8 py-4 bg-blue-500 text-white rounded-lg shadow-md hover:bg-blue-600 transition-all cursor-pointer"
        >
          Users
        </button>
        <button
          onClick={clickHandler}
          value="tasks"
          classnameName="px-8 py-4 bg-green-500 text-white rounded-lg shadow-md hover:bg-green-600 transition-all cursor-pointer"
        >
          Tasks
        </button>
        <button
          onClick={clickHandler}
          value="status"
          classnameName="px-8 py-4 bg-yellow-500 text-white rounded-lg shadow-md hover:bg-yellow-600 transition-all cursor-pointer"
        >
          Status
        </button>
        <button
          onClick={clickHandler}
          value="addTask"
          classnameName="px-8 py-4 bg-purple-500 text-white rounded-lg shadow-md hover:bg-purple-600 transition-all cursor-pointer"
        >
          Add Task
        </button>
    ...
    <div id="fullWidthTabContent" classname="border-t border-gray-200 dark:border-gray-600">
      ...
      {/* <div classnameName="mt-8 w-full">
        {rData === null ? (
          <p classnameName="text-center text-gray-600">Click a button to load data...</p>
        ) : rData.length > 0 ? (
          <Getallusers value={rData} />
        ) : (
          <p classnameName="text-center text-red-500">No user data available</p>
        )}
      </div> */}
    </div>
  );
