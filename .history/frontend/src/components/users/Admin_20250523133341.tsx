...
const Admin = () => {
  ...
  const allTasks = users.flatMap(user => user.tasks_tasks_user_idTousers || []);
  ...
  const miniPie = (label, count, color) => (
    <div className="flex flex-col items-center gap-2">
      <div className="w-24 h-24">
        <ResponsiveContainer>
          <PieChart>
            <Pie
              data={[{ value: count }, { value: totalTasks - count }]}
              dataKey="value"
              innerRadius={30}
              outerRadius={40}
              startAngle={90}
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-gray-500">{count} / {totalTasks} ({getPercentage(count)}%)</p>
      </div>
    </div>
  );
      ...
      {isSuccess && (
        <div className="bg-white shadow-xl rounded-2xl p-6 border border-gray-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-4">
            {miniPie('Draft', draft, COLORS.draft)}
            {miniPie('In Progress', inProgress, COLORS['in-progress'])}
            {miniPie('Completed', completed, COLORS.completed)}
          </div>
        </div>
      )}
    </div>
