...
const Status: React.FC = () => {
  ...
  console.log(allTasks, "fromtasks")
  ...
  //       console.log(response.data.tasks,"from first api")
    ...
    <div className="min-h-screen p-6 bg-gray-100">
      ...
      <div className="max-w-6xl mx-auto  rounded-lg p-6">
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          {['All', 'Draft', "Pending", 'Completed'].map((stat) => (
            <button
              key={stat}
              onClick={() => handleStatusFilter(stat)}
              className={`py-2 px-4 rounded font-semibold ${status === stat ? 'bg-indigo-600 text-white' : 'bg-gray-200 hover:bg-gray-300'
                }`}
            >
              {stat}
            </button>
      ...
      ${task.priority === "high"
                    ...
                    <td className="border px-4 py-2 capitalize">{task.eta ? task.eta : "Null"}</td>
                  </tr>
                ))
              ) : (
                <tr>
