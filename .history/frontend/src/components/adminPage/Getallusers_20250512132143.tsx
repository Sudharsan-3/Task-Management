...
const Getallusers: React.FC = () => {
  ...
  const { user } = useContext(AuthContext) as { user: string }; // ✅ user is the JWT token string
    ...
    const fetchUsers = async () => {
      try {
        const response = await api.get('/api/allUsers', {
          headers: {
            Authorizationuthorization: user, // ✅ JWT token directly in the header
          },
        });
...
//     alert(`User Details:\n\nName: ${user.name}\nEmail: ${user.email}\nRole: ${user.role}`);
//   };
//   console.log(user,"from getalluser")
...
//           headers: {
//             Authorization: user.token, // ✅ No "Bearer", just the token
//           },
//         });
//         setUsers(response.data);
//         setNotification(`Successfully loaded ${response.data.length} users`);
