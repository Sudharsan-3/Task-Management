...
const LoadTasks: React.FC = () => {
  ...
  const [showSuccessOnLoad, setShowSuccessOnLoad] = useState(false);
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      if (hasIncompleteTasks) {
        toast.error('Some tasks missed their deadlines!');
      }
    }
  ...
  }, [isSuccess, isError, data, showSuccessOnLoad]);

  const handleShow = (task: Task) => {
    navigate("/View-Task", { state: { task: task } });
  };
...
//       const hasPendingTasks = tasks.some((task) => task.status.toLowerCase() === 'pending');
//       if (hasPendingTasks) {
