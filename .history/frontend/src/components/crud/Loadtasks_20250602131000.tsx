...
const LoadTasks: React.FC = () => {
  ...
  useEffect(() => {
    if (isSuccess && data?.tasks) {
      ...
      const hasMissedDeadlines = data.tasks.some((task: Task) => {
  ...
  const handleShow = (task: Task): void => {
  ...
  const filteredTasks = data?.tasks?.filter((task: Task) => {
    ...
    // Match by task name
    const matchName = task.task_name.toLowerCase().includes(normalizedQuery);
  
    // Match by ID
    const matchID = task.id.toString().includes(normalizedQuery);
  
    // Match by custom ticket format like: TICK-DATE(020625)-001
    let matchTicket = false;
  
    const ticketRegex = /^t(i)?ck(-date\((\d{6})\))?-(\d{1,})$/i;
    const ticketMatch = normalizedQuery.match(ticketRegex);
  
    if (ticketMatch) {
      const datePart = ticketMatch[3]; // 020625
      const idPart = parseInt(ticketMatch[4]); // 001
  
      const matchIDExact = task.id === idPart;
  
      let matchDate = false;
      if (datePart && task.eta) {
        ...
        const yy = String(taskDate.getFullYear()).slice(-2);
        const taskDateCode = `${dd}${mm}${yy}`; // matches 020625
        matchDate = datePart === taskDateCode;
      }
  
      matchTicket = matchIDExact && (matchDate || !datePart);
    }
  
    return matchesStatus && (matchName || matchID || matchTicket);
  });
  
  // const filteredTasks: Task[] | undefined = data?.tasks?.filter((task: Task) => {
  //   const matchesStatus =
  //     filterStatus === 'all'
  //       ? true
  //       : filterStatus === 'incomplete'
