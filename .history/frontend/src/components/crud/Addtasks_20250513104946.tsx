...
const CreateTask: React.FC = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-6 rounded-md shadow-md mt-6">
      ...
      <button
        onClick={() => navigate(-1)}
        className="mb-4 px-4 py-2 text-sm rounded-md bg-gray-200 hover:bg-gray-300 transition"
      >
        ← Back
      </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label name={sdf} className="block font-medium text-gray-700 mb-1">Select User</label>
          <select
            required
            value={selectedUserId}
            onChange={(e) =>{ setSelectedUserId(e.target.value);setUsername(e.target.value)}}
            className="w-full border rounded-md px-4 py-2"
          >
            <option value="">-- Select User --</option>
            {users.map((user) => (
              <option key={user.id} value={user.id}> name={user.name} {user.name}</option>
            ))}
          </select>
        </div>
