...
const CreateTask: React.FC = () => {
  ...
  const { data: usersData, isLoading: usersLoading, isSuccess: usersSuccess, isError: usersError } = useQuery({
    queryKey: ['users'],
    queryFn: fetchUsers,
  });
  ...
  }, [usersLoading, usersSuccess, usersError, usersData]);
  ...
  const createTaskMutation = useMutation({
    ...
    onMutate: () => {
      toast.loading("Creating task...", { toastId: "creating-task" });
    },
    onSuccess: () => {
      toast.dismiss("creating-task");
      toast.success("Task created successfully!");
      queryClient.invalidateQueries(['userTasks']); // optional: refresh task list
      setTaskName('');
      setTaskDescription('');
      setPriority('medium');
      setStatus('draft');
...
//     }
//     const {data,isLoading,isSuccess,isError} = useQuery({
//       queryKey:["featchData1",user.name],
//       queryFn:featchData1
//     })
...
//         toast.dismiss("fetching");
//         toast.success("Task Created successfully!");
//         console.log(data.tasks.eta, "from status second api")
//         setTaskName('');
//         setTaskDescription('');
//         setPriority('medium');
//         setStatus('draft');
