...
const User = () => {
  ...
  const [selectedDate, setSelectedDate] = useState(""); // ETA date filter
  ...
  // Filtered tasks by status and date
  const filteredTasks =
    data?.data?.tasks?.filter((task: any) => {
      const matchesStatus =
        filter === "All" || task.status === filter.toLowerCase();

      const matchesDate = selectedDate
        ? new Date(task.eta).toISOString().split("T")[0] === selectedDate
        : true;
