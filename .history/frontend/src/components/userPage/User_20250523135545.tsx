          ...
          <table className="min-w-full text-sm text-black border-collapse">
            ...
            <tbody>
              {data?.data.tasks.map((task: any, index: number) => {
                ...
                const etaMatch = desc.match(/ETA:\s*([^\.\n]+)/i);
                const commentMatch = desc.match(/User Comment:\s*([^\.\n]+)/i);
                const mainDesc = desc.split("ETA:")[0].trim();
                ...
                return (
                  <tr key={task.id} className="hover:bg-gray-50">
                    ...
                    <td className="border px-4 py-2">
                      <p className="mb-1">{mainDesc}</p>
                      {etaMatch && (
                        <p className="text-xs text-gray-600">
                          <strong>ETA:</strong> {etaMatch[1]}
                        </p>
                      )}
                      {commentMatch && (
                        <p className="text-xs text-gray-600">
                          <strong>User Comment:</strong> {commentMatch[1]}
                        </p>
                      )}
                    </td>
                    <td className="border px-4 py-2">
