...
const Responsetask: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      if (!formattedEta) {
        toast.error("Please provide the ETA");
        return;
      }

      await api.put("/api/updateusertask", {
        id: task?.id,
        user_comments: description.trim() === "" ? null : description,
        status: "pending",
        eta: formattedEta,
      });
  ...
  const handleAddToCalendar = () => {
    if (!eta) {
      toast.error("ETA is missing. Please provide an ETA before adding to calendar.");
      return;
    }
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <div className="max-w-2xl mx-auto bg-white p-6 rounded shadow space-y-4">
        <div className="flex gap-2">
          <h4 className="font-semibold text-gray-700">Ticket:</h4>
        ...
        {eta && (
          <div className="text-right mt-2">
            <button
              ...
              className="bg-green-600 w-full text-white px-4 py-2 rounded hover:bg-green-700 hover:cursor-pointer"
            >
...
//   const handleSubmit = async () => {
//     try {
//       if (!formattedEta || !status) {
//         toast.error("Please provide both eta and status");
//         return;
//       }
...
//     const start = new Date(); // When the user is submitting ETA
//     const end = new Date(eta); // ETA set by the user
