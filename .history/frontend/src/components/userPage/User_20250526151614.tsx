...
const User = () => {
  ...
  useEffect(() => {
    ...
    if (isSuccess && data?.data?.tasks?.length > 0) {
      toast.dismiss("fetching");
      ...
      // ⚠️ Check for pending tasks
      const hasPendingTasks = tasks.some((task:string|any) => task.status === "pending");
      if (hasPendingTasks) {
        toast.warn("⚠️ You have pending tasks to complete!", { toastId: "pending-tasks" });
      }
  
      // ⚠️ Check for overdue incomplete or pending tasks
      const overdueTasks = tasks.filter((task:string|any) =>
        (task.status === "pending" || task.status === "incomplete") &&
        new Date(task.eta) < now
      );
  
      if (overdueTasks.length > 0) {
        toast.error("⏰ You have overdue tasks that are not completed!", {
          toastId: "overdue-tasks",
        });
      }
    }
