...
import Navigatetask from "./Navigatetask";
...
const User = () => {
  ...
  // Check ETA and update status if needed
  useEffect(() => {
    if (data?.data?.tasks?.length) {
      ...
      const updatedTasks = data.data.tasks.map((task: any) => {
        ...
        // Update to incomplete if past ETA and not completed
  ...
  return (
    <div className="min-h-screen bg-white p-4 sm:p-6">
      ...
      {filteredTasks?.length > 0 ? (
        ...
        //   </table>
        // </div>
        <Navigatetask filteredTasks={filteredTasks} getStatusColor={getStatusColor} />
      ) : (
