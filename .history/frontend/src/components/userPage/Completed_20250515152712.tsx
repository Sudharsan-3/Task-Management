  ...
  return (
    <div className="min-h-screen bg-white p-4 sm:p-6">
      ...
      {data?.data?.tasks?.length > 0 ? (
        <div className="overflow-auto rounded-md border border-gray-300 shadow-sm">
          <table className="min-w-full text-sm text-gray-700">
            <thead className="bg-gray-100">
              <tr>
                <th className="text-left p-3 border-b">S.No</th>
                <th className="text-left p-3 border-b">Name</th>
                <th className="text-left p-3 border-b">Task Name</th>
                <th className="text-left p-3 border-b">Priority</th>
                <th className="text-left p-3 border-b">Status</th>
                <th className="text-left p-3 border-b">User Comment</th>
              </tr>
            </thead>
            <tbody>
              {data.data.tasks.map((task: any, index: number) => (
                <tr key={task.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-3 border-b capitalize">{task.user_name}</td>
                  <td className="p-3 border-b">{task.task_name}</td>
                  <td className="p-3 border-b capitalize">{task.priority}</td>
                  <td className="p-3 border-b capitalize">{task.status}</td>
                  <td className="p-3 border-b">{task.user_comments || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
