  ...
  useEffect(() => {
    ...
    if (isSuccess) {
      ...
      const hasPending = tasks.some(
        (task: any) => task.status.toLowerCase() === "pending"
      );

      const hasOverdueIncomplete = tasks.some((task: any) => {
        if (!task.eta || task.status.toLowerCase() === "completed") return false;
        return new Date(task.eta) < now && task.status.toLowerCase() === "incomplete";
      });

      const hasOverduePending = tasks.some((task: any) => {
        return new Date(task.eta) < now && task.status.toLowerCase() === "pending";
      });

      if (hasPending) {
        toast.warn("⏳ You have pending tasks to complete!");
      }

      if (hasOverdueIncomplete) {
        toast.error("⚠️ You have overdue incomplete tasks!");
      }

      if (hasOverduePending) {
        toast.warn("⚠️ You have pending tasks past their deadline!");
      }
    }
